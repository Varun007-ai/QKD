# QuantumVerse - High-Dimensional BB84 Quantum Blockchain with Enhanced Security
# Implemented with comprehensive security improvements and optimizations
...
<style>
    ...
    @keyframes payment-confirmed {
        ...
        50% {
            transform: scale(1.02);
            box-shadow: 0 0 20px 10px rgba(67, 233, 123, 0.4);
        }
        100% {
            transform: scale(1);
    ...
    .transaction-confirmed {
        animation: payment-confirmed 1.5s ease-in-out;
    }
...
if "logged_in_user" in st.session_state:
    ...
    current_balance = st.session_state.balances.get(user_wallet["public_key"], 0.0)
    
    st.markdown(f"""
    <div class="top-nav">
        <div class="nav-container">
            <div class="nav-logo"> QuantumVerse</div>
            <div class="nav-items">
                <div class="nav-item {'active' if st.session_state.current_tab == 'wallet' else ''}" 
                     onclick="switchTab('wallet')"> Wallet</div>
                <div class="nav-item {'active' if st.session_state.current_tab == 'analytics' else ''}" 
                     onclick="switchTab('analytics')"> Analytics</div>
                <div class="nav-item {'active' if st.session_state.current_tab == 'network' else ''}" 
                     onclick="switchTab('network')"> Network</div>
                <div class="nav-item {'active' if st.session_state.current_tab == 'market' else ''}" 
                     onclick="switchTab('market')"> Market</div>
                <div class="nav-item {'active' if st.session_state.current_tab == 'security' else ''}" 
                     onclick="switchTab('security')"> Security</div>
                <div class="nav-item {'active' if st.session_state.current_tab == 'settings' else ''}" 
                     onclick="switchTab('settings')"> Settings</div>
            </div>
        </div>
        <div class="user-section">
            <div class="user-badge">
                 {user_wallet['username']} | {current_balance:.2f} QCoins
            </div>
...
function switchTab(tabName) {
    // This will trigger a Streamlit rerun with the new tab value
...
function logout() {
    // This will trigger a Streamlit rerun with logout action
    window.parent.postMessage({
        type: 'streamlit:setComponentValue',
        value: 'logout'
    }, '*');
...
if "logged_in_user" not in st.session_state:
    ...
    tabs = st.radio(
        "Select Tab",
        ["wallet", "analytics", "network", "market", "security", "settings"],
    ...
    # WALLET TAB
    if st.session_state.current_tab == "wallet":
        # User overview cards
                ...
                if amount > 0 and (amount + fee) <= current_balance:
                    ...
                    if result:
                        ...
                        <div style="text-align: center; margin: 2rem 0;">
                            ...
                            <p style="margin-top: 1rem; font-weight: 600; color: #43e97b;">Payment Confirmed!</p>
                        </div>
                        """, unsafe_allow_html=True)
                    ...
                    <div style="display: flex; justify-content: space-between; align-items: flex-start;">
                        <div style="flex: 1;">
                            <div style="font-weight: 600; font-size: 1.1rem; margin-bottom: 0.5rem;">
                                {direction_icon} {direction.title()} - Block #{tx["block"]}
                            </div>
                            <div style="color: rgba(255,255,255,0.7); margin-bottom: 0.8rem;">
                                {tx["type"].title()} • {tx["timestamp"][:19]}
                            </div>
                            <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.8rem; color: rgba(255,255,255,0.6); margin-bottom: 0.8rem;">
                                ID: {tx.get("tx_id", "N/A")}
                            </div>
                            {quantum_info}
                        </div>
                        <div style="text-align: right; min-width: 100px;">
                            <div style="color: {amount_color}; font-weight: 700; font-size: 1.4rem; margin-bottom: 0.2rem;">
                                {"-" if direction == "sent" else "+"}{float(tx["amount"]):.2f}
                            </div>
...
st.markdown("""
<script>
// Listen for messages from the navigation
window.addEventListener('message', function(event) {
    if (event.data.type === 'streamlit:setComponentValue') {
        // This would set the tab value in Streamlit
        // In a real implementation, we'd use Streamlit's setComponentValue
        console.log('Tab change requested:', event.data.value);
    }
});
</script>
""", unsafe_allow_html=True)
